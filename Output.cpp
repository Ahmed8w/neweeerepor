...
void Output::DrawFlag(const CellPosition& cellPos) const //ALI
{										
	// TODO: Validate the cell position
	if (cellPos.IsValidCell()) {
		...
		// 		 2. Draw the flag (the triangle)
			int triangleCenterX = flagPoleStartX + UI.FlagPoleWidth / 2;
			int triangleCenterY = flagPoleEndY;
			int triangleHeight = UI.FlagHeight;
			int triangleWidth = UI.FlagWidth;
			pWind->SetPen(UI.FlagColor);
			pWind->SetBrush(UI.FlagColor);
			DrawTriangle(triangleCenterX, triangleCenterY, triangleHeight, triangleWidth, RIGHT, UI.FlagColor, FILLED, 1);
...
void Output::DrawDangerZone(const CellPosition& cellPos) const //ALI
{	
	
	std::cout << cellPos.GetCellNum() << std::endl;
	...
	if (cellPos.IsValidCell()) {
		...
		pWind->SetBrush(RED);
		pWind->SetPen(RED);
		pWind->DrawRectangle(cellStartX, cellStartY, cellStartX + cellWidth, cellStartY + cellHeight);
	}
	else {return;}
}


void Output::DrawWaterPit(const CellPosition& cellPos) const //ALI
{
	///TODO: Complete the implementation of the following function
	if (cellPos.IsValidCell()) {
		int cellStartX = GetCellStartX(cellPos);
